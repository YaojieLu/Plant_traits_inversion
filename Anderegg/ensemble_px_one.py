...
def qtf(traces, T, I, D, ps, Jmax, Vcmax, n_samples=1000):
    ...
    for i in range(len(samples)):
        px = muf(samples.iloc[i], T, I, D, ps, Jmax, Vcmax)
        df_px.append(px)
...
def f1(species):
    ...
    # run    
    px = qtf(ts, T, I, D, ps, Jmax, Vcmax)
    px['date'] = list(df_sp['date'])
    px['ps'] = list(df_sp['ps'])
    px.rename(columns={0.05: 'qt=0.05', 0.5: 'qt=0.5', 0.95: 'qt=0.95'},\
              inplace=True)
    return px

# run
px = f1('Bpa')
px.to_csv('../Results/ensemble_px_UMB_Bpa.csv', index=False)

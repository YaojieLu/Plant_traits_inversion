...
# Sobol parameters
paras = ['alpha_log10', 'c_log10', 'g1_log10', 'kxmax_log10', 'p50']
...
def testf(X, env, Vcmax=60, Jmax=120, ca=400, Kc=460, q=0.3, R=8.314, z1=0.9, z2=0.9999, a=1.6, L=1):
    alpha_log10, c_log10, g1_log10, kxmax_log10, p50 = X
...
for d in Days:    
    ...
    for j, X in enumerate(par1):
        if testf(X, df[['T', 'I', 'D', 'ps', 'day_len']].iloc[d, :]) >= 0:
            badindex.append(j)
    ...
    Y = []
    for X in par2:
        Y.append(muf(X, df[['T', 'I', 'D', 'ps', 'day_len']].iloc[d, :]))
...
## Save to CSV
#dfS1.to_csv('MCMC_outputs/S1_v2_ps_input.csv', index=False)
#dfST.to_csv('MCMC_outputs/ST_v2_ps_input.csv', index=False)

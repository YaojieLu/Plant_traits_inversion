...
params = ['alpha', 'g1', 'kxmax', 'L']
latex = ['$\\alpha$', '$\\mathit{g_1}$', '$\\mathit{k_{xmax}}$', '$\\mathit{L}$']
#latex = ['$\\alpha$', '$\\mathit{c}$', '$\\psi_{x50}$',
#         '$\\mathit{k_{xmax}}$', '$\\mathit{g_1}$', '$\\mathit{L}$']
df_thinned = {}
for key in params:
    df_thinned[key] = [item for index, item in enumerate(ts[key]) if index % 15 == 0]
df_thinned = pd.DataFrame.from_dict(data = df_thinned, orient = 'columns')
df_thinned.rename(columns = dict(zip(params, latex)), inplace = True)
# mutual information
def mif(x, y, **kwargs):

...
traces = []
for s in species_name_dict.keys():
    traces.append(pickle.load(open('../../Data/UMB_trace/{}.pickle'.format(s), 'rb'))['p50'])
traces_thinned = []
for t in traces:
    traces_thinned.append([item for index, item in enumerate(t) if index % 10 == 0])
...
plt.tight_layout
plt.savefig('../../Figures/Figure UMB p50.png', bbox_inches='tight')
